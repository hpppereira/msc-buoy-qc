...
mpl.reload(proconda)
mpl.reload(consiste_bruto)
mpl.reload(consiste_espec)
mpl.reload(consiste_proc)
mpl.reload(daat)
mpl.reload(relatorio)
mpl.reload(graficos_axys)
# reload(jonswap_wafo)
mpl.reload(jonswap)
...
h = 200 #profundidade 
nfft = 164 #numero de dados para a fft (p/ nlin=1312: 32gl;nfft=82, 16gl;nfft=164, 8gl;nfft=328)
...
for i in range(len(listap)):
    ...
    flagb[i,1] = consiste_bruto.shift(eta,164,8,0.3,flagb[i,1])
    flagb[i,2] = consiste_bruto.shift(etax,164,8,0.3,flagb[i,2])
    flagb[i,3] = consiste_bruto.shift(etay,164,8,0.3,flagb[i,3])
...
if mdp == 1:
   ...
   ##############################################################################
    ...
    pl.plot([t1[0],t1[-1]],[-5*np.std(eta),-5*np.std(eta)],'r-.',linewidth=3)
    pl.plot([t1[0],t1[-1]],[-4*np.std(eta),-4*np.std(eta)],'r-.',linewidth=3)
    pl.title(datat.astype(str)[0][:-6]+'h\n Hs=%.1f' %matondap[0,1]+' m, Hmax=%.1f' %matondap[0,3]+' m, Hmax/Hs=%.1f' %(matondap[0,3]/matondap[0,1])+' m, Tp=%.1f' %matondap[0,7]+' s, Dp=%i' %(matondap[0,8]-17)+u' \u00b0')
    pl.axis('tight')
    pl.xlabel(r'$Time\ (s)$',fontsize=15)
    pl.ylabel(r'$Heave\ (m)$',fontsize=15)
    pl.grid()
    pl.yticks([-8,-6,-4,-2,0,2,4,6,8])
    pl.ylim(-9,9)
    ...
    dmag = -17
    dire1[pl.find(dire1 + dmag < 0)] = dire1[pl.find(dire1 + dmag < 0)] + 360
    pl.subplot(2,2,3)
    pl.plot(sn[:,0],dire1+dmag)
    pl.axis('tight')
    pl.ylim(0,360)
    pl.yticks([0,45,90,135,180,225,270,315,360])
    pl.plot([1/tp,1/tp],[0,360],'k--')
    pl.xlabel(r'$Frequency\ (Hz)$',fontsize=15)
    pl.ylabel(r'$Direction\ ($' + u'\u00b0' + r'$)$',fontsize=15)
    pl.grid()
    ...
    pl.axis('tight')
    pl.xlabel(r'$Frequency\ (Hz)$',fontsize=15)
    pl.ylabel(r'$m^{2}/Hz$',fontsize=15)
    pl.grid()
